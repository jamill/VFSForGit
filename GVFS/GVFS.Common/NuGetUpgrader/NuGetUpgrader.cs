        ...
        public bool TryRunInstaller(InstallActionWrapper installActionWrapper, out string error)
        {
            ...
            using (ITracer activity = this.tracer.StartActivity(nameof(this.TryRunInstaller), EventLevel.Informational))
            {
                try
                {
                    ...
                    InstallManifestPlatform platformInstallManifest = this.releaseManifest.PlatformInstallManifests[ReleaseManifest.WindowsPlatformKey];
                    activity.RelatedInfo($"Extracted ReleaseManifest from JSON. InstallActions: ${platformInstallManifest.InstallActions.Count}", EventLevel.Informational);
                    ...
                    foreach (ManifestEntry entry in platformInstallManifest.InstallActions)
                    {
                        ...
                        activity.RelatedInfo(
                            ...
                            $"InstallerPath: {installerPath} Args: {entry.Args} ", EventLevel.Informational);
                ...
                {
                    activity.RelatedError($"Install actions completed successfully.");
                    error = null;
                    return true;
                }
            }
