...
namespace GVFS.Platform.Mac
{
    public partial class MacPlatform : POSIXPlatform
    {
        ...
        public override void IsServiceInstalledAndRunning(string name, out bool installed, out bool running)
        {
            ...
            MacServiceProcess macServiceProcess = new MacServiceProcess();
            List<MacServiceProcess.ServiceInfo> services;
            if (!macServiceProcess.TryGetServices(currentUser, out services, out string error))
            {
                installed = false;
                running = false;
            }

            MacServiceProcess.ServiceInfo gvfsService = services.FirstOrDefault(sc => string.Equals(sc.Name, "org.vfsforgit.service"));
