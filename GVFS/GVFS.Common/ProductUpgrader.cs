...
namespace GVFS.Common
{
    ...
    public abstract class ProductUpgrader : IDisposable
    {
            ...
            out string error)
        {
            ...
            {
                // We are configured for NuGet - determine if we are using ModernNuGetUpgrader or not
                if (containsOrgInfoServerUrl)
                {
                    if (ModernNugetUpgrader.TryCreate(
                        tracer,
                        fileSystem,
                        gvfsConfig,
                        new HttpClient(),
                        ...
                        dryRun,
                        noVerify,
                        out ModernNugetUpgrader modernNuGetUpgrader,
                        out error))
                    {
                        // We were successfully able to load a NuGetUpgrader - use that.
                        newUpgrader = modernNuGetUpgrader;
                        return true;
                    }
                    else
                    {
